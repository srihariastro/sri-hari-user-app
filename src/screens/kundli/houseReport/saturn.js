...
const SaturnHouse = ({ navigation, houseReportData }) => {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View
          ...
          <View style={styles.itemContainer}>
            <Text allowFontScaling={false} style={styles.itemText}> {houseReportData?.saturnReports?.planet}</Text>
            <View>
              <Text allowFontScaling={false} style={{ color: 'black', padding: 10, fontSize: getFontSize(1.8), textAlign: 'justify' }}>
              {houseReportData?.saturnReports?.house_report}
              </Text>
            </View>
          </View>
...
export default connect(mapStateToProps, null)(SaturnHouse);
