...
import { connect } from 'react-redux';
  const MarsHouse = ({ navigation, houseReportData }) => {
            ...
            style={{
              ...
              elevation:6
                ...
                <View style={styles.itemContainer}>
                    <Text allowFontScaling={false} style={styles.itemText}>  {houseReportData?.marsReports?.planet}</Text>
                    <View>
                    <Text allowFontScaling={false} style={{ color: 'black', padding: 10, fontSize: getFontSize(1.8), textAlign: 'justify' }}>
                    {houseReportData?.marsReports?.house_report}
                    </Text>
                    </View>
                </View>
  ...
  export default connect(mapStateToProps, null)(MarsHouse);
