    ...
    >
      <View>
        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5 }}>
          ...
          <Text allowFontScaling={false} style={{ flex: 1, color: "#999", fontSize: 13 }}>{convertToXTimeAgo(note.updatedAt)}</Text>
          {note.isBookmarked && (
        ...
        {note.labels.length > 0 && (
          <ScrollView horizontal contentContainerStyle={{ flexDirection: "row", alignItems: "center", paddingTop: 5 }}>
            {note.labels?.map(label => (
              <Text allowFontScaling={false} key={label} style={{ marginRight: 10, padding: 5, backgroundColor: "#eee", color: "#666", borderRadius: 3, fontSize: 13 }}>{label}</Text>
            ))}
          </ScrollView>
        )}

        {note.reminder?.dateTime && (
          <Text allowFontScaling={false}>+ Reminder</Text>
        )}

        <Text allowFontScaling={false} numberOfLines={5} style={{ color: "#555", fontSize: 15, marginTop: 10 }}> {note.text} </Text>
      </View>
    </Pressable>
  );
}
      ...
      {trashNotes.length > 0 && (
        <>
          <View style={{ flexDirection: "row", alignItems: "center", marginVertical: 20, marginHorizontal: 20 }}>
            <Text allowFontScaling={false} style={{ color: colors.background_theme2, fontWeight: "600", fontSize: 15 }}>{trashNotes.length} note{trashNotes.length > 1 && 's'} in trash</Text>
            <Pressable
              ...
              android_ripple={{ color: "#bbb", radius: 200 }}
            >
              <Text allowFontScaling={false}>Restore</Text>
            </Pressable>
            <Pressable
            ...
            >
              <Text allowFontScaling={false} style={{ color: "white" }}>Empty</Text>
            </Pressable>
          </View>
      ...
      <Modal visible={noteModalId !== null} onRequestClose={() => setNoteModalId(null)}>
        <View>
          <Pressable
            ...
            android_ripple={{ color: "#bbb", radius: 200 }}>
            <Text allowFontScaling={false} style={{ color: "blue", textAlign: "center" }}>Restore</Text>
          </Pressable>
          ...
          <Pressable
            ...
            style={{ paddingVertical: 10 }}
            android_ripple={{ color: "#bbb", radius: 200 }}>
            <Text allowFontScaling={false} style={{ color: "red", textAlign: "center" }}>Delete permanently</Text>
          </Pressable>
        </View>
      </Modal>
    </View>
