import {
  ...
  const WalletGST = props => {
    ...
    const [firstOffer, setFirstOffer] = useState(props.route.params.data2);
          ...
          <View style={{ flex: 1, width: '90%', alignSelf: 'center' }}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.rowContainer}>
                ...
                <Text allowFontScaling={false} style={styles.rowText}>
                  ₹ {parseFloat(walletData * 0.18)}
                </Text>
              </View>
              <View style={styles.rowContainer}>
                <Text allowFontScaling={false} style={styles.rowText}>Gift Amount</Text>
                <Text allowFontScaling={false} style={styles.rowText}>
                  ₹ {parseFloat(firstOffer).toFixed(1)}
                </Text>
              </View>
              <View style={styles.rowContainer}>
                <Text allowFontScaling={false} style={styles.rowText}>Total Payable Amount</Text>
                <Text allowFontScaling={false} style={styles.rowText}>
                  ₹ {parseFloat(walletData) + parseFloat(walletData * 0.18)}
                </Text>
              </View>
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => phonepe(parseFloat(walletData) + parseFloat(walletData * 0.18))}
                style={{
                  flex: 0,
